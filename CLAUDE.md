...
| ブランチ | デプロイ先 | コマンド |
|---------|-----------|---------|
| `staging` | ステージング（lifewell-shippingtaskapp-test） | `npm run deploy:staging` |
| `main` | 本番（lifewell-shippingtaskapp） | `npm run deploy` |

### 必須ルール（厳守）

- **作業は常に `staging` ブランチで行う。** ユーザーが特に指定しない限り、コード変更・デプロイはすべて staging。
- **「本番に反映して」という明示的な指示があった場合のみ** `staging` → `main` へマージして本番デプロイを行う。
- 本番デプロイ前は必ず「本番（main）にデプロイします。よろしいですか？」と確認を取る。
- デプロイ前に `git branch` で現在のブランチを確認する。
- ブランチが `main` になっていたら、作業前に必ずユーザーに確認する。
